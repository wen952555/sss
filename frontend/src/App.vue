<template>
  <div id="app-container" class="app-flex-container">
    ...
    <div class="game-area app-flex-grow">
      ...
      /> <!-- GameBoardComponent is self-closing -->
...
<script setup>
// Script part is identical to the last successfully built version.
...
watch(playerArrangedHand, (newVal) => {
  // console.log('WATCH playerArrangedHand:', 
  //   `F:${newVal.front.map(c=>c.id).join(',')}(${newVal.front.length})`, 
  //   `M:${newVal.middle.map(c=>c.id).join(',')}(${newVal.middle.length})`, 
  //   `B:${newVal.back.map(c=>c.id).join(',')}(${newVal.back.length})`
  // );
}, { deep: true });


const validationMessage = computed(() => {
  // console.log("VALIDATION: Computing validationMessage...");
  const frontCards = playerArrangedHand.front;
  const middleCards = playerArrangedHand.middle;
  const backCards = playerArrangedHand.back;

  if (frontCards.length !== 3) return `头墩需3张牌 (当前${frontCards.length}张)`;
  if (middleCards.length !== 5) return `中墩需5张牌 (当前${middleCards.length}张)`;
  if (backCards.length !== 5) return `尾墩需5张牌 (当前${backCards.length}张)`;

  const allDuns = [...frontCards, ...middleCards, ...backCards];
  const allCardIdsInDuns = new Set();
  
  for (const card of allDuns) {
      if (!card || typeof card.id === 'undefined') {
          return "墩内存在无效的卡片数据"; 
      }
      if (allCardIdsInDuns.has(card.id)) {
          return `牌 ${card.id} 在不同墩中重复出现`;
      }
      allCardIdsInDuns.add(card.id);

      if (!playerHandInitial.value.find(initialCard => initialCard.id === card.id)) {
          return `墩内包含非初始手牌 (${card.id})`;
      }
  }

  if (allCardIdsInDuns.size !== 13) {
    return `总牌数不为13 (当前${allCardIdsInDuns.size}张)`;
  }
  return "可以提交";
});
...
function rankCard(card) {
    ...
    if (!card || typeof card.value === 'undefined' || typeof card.suit === 'undefined') return -1;
...
function submitPlayerHand() {
  if (validationMessage.value !== "可以提交") {
    generalError.value = "提交失败: " + validationMessage.value;
    return;
  }
