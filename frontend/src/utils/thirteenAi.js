...
// --- 牌型代码 ---
// 这些常量将被 evaluateHandSimple 正确使用，不应再报 unused
...
// 确保此函数被导出，并且内部调用了 prepareCardsForEval 并使用了 HAND_TYPE_... 常量
export function evaluateHandSimple(cardObjects) {
    // (这里的 evaluateHandSimple 函数体与上一次能通过 ESLint (除了HAND_TYPE常量) 的版本完全一致。
    //  它内部调用了 prepareCardsForEval，并使用了所有 HAND_TYPE_... 常量。
...
// --- 您提供的 AI 辅助函数 ---
// --- 修改点：为所有未在 smartAiArrangeCards 中被明确调用的辅助函数添加 eslint-disable-next-line ---
...
export function smartAiArrangeCards(allCardsInput) {
    ...
    // ... (确保这个函数内部确实调用了它所依赖的所有辅助函数，否则上面的 eslint-disable 是必要的)
