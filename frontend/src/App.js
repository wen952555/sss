...
function App() {
  ...
  // const [gameMessage, setGameMessage] = useState(''); // 彻底移除 gameMessage state
  ...
  useEffect(() => { 
    ...
    // 不再通过 gameMessage 更新提示，UI状态通过按钮和AI状态栏反映
  }, [humanPlayer, checkPlayerFullyArranged, players]); // 依赖中移除 gameMessage 相关的
  ...
  const handleDealNewHand = async () => { 
    ...
    try { 
      ...
      // 可以考虑用一个非常临时的alert或不那么侵入式的方式提示错误
      alert(`发牌请求错误: ${error.message}`);
  ...
  useEffect(() => { /* 初始不发牌，也不设置 gameMessage */ }, []);
    ...
    <DragDropContext onDragEnd={onDragEnd}>
      <div className="app-container">
        ...
        <ComparisonModal 
            ...
            onClose={() => {setShowComparisonModal(false); /* 不再设置gameMessage */}}
            players={players}
