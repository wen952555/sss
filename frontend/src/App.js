...
function App() {
  ...
  const [gameMessage, setGameMessage] = useState(''); // 初始化为空
  const [allPlayersArranged, setAllPlayersArranged] = useState(false);
  const [showComparisonModal, setShowComparisonModal] = useState(false);
  const [gameStarted, setGameStarted] = useState(false); // 新增状态，标记游戏是否已开始（发牌后）
  ...
  useEffect(() => {
    if (showComparisonModal) { // 如果模态框打开，不显示主界面的 gameMessage
      setGameMessage(''); 
      return;
    }
    ...
    if (!humanPlayer) return; // 如果 humanPlayer 还未初始化，不继续
    ...
    if (isHumanArranged) {
      if (humanPlayer.isMisArranged) {
        ...
        if (allDone) {
            // 当所有人都准备好时，gameMessage 可以清空，因为“开始比牌”按钮会出现
            setGameMessage(""); // 清空，或一个非常简短的提示
        } else {
    ...
    // else if (!isLoadingDeal) { // 移到 useEffect 开始处判断 gameStarted
    //     setGameMessage('点击“重新发牌”开始游戏');
    // }
  ...
  // --- (handleDealNewHand, autoArrangeAIHand, onDragEnd, handleHumanPlayerAISort, handleConfirmHand, handleStartCompare - 保持不变，但 handleDealNewHand 中设置 gameStarted) ---
  const handleDealNewHand = async () => { 
    ...
    try { 
        ...
        setGameStarted(false); // 发牌失败，重置游戏开始状态
    } 
  ...
  useEffect(() => { if (!gameStarted && !isLoadingDeal) { setGameMessage('点击“重新发牌”开始游戏'); } }, [gameStarted, isLoadingDeal]); // 更新初始提示的条件
  ...
  // ... (计算 humanMiddleCurrentCount, humanTopCurrentCount, humanBottomCurrentCount, humanMiddleExpectedCount 的逻辑保持不变) ...
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      <div className="app-container">
        ...
        {/* 修改 gameMessage 的渲染条件：只有当 gameMessage 有实际内容（不是初始提示），且模态框未打开时才显示 */}
        ...
        {/* 仅在游戏未开始（未发牌）且不在加载中时，显示初始提示 */}
        {!gameStarted && !isLoadingDeal && (
            <p className="game-message initial-prompt">点击“重新发牌”开始游戏</p>
        )}
        ...
        <ComparisonModal 
            ...
            onClose={() => {setShowComparisonModal(false); setGameMessage(gameStarted ? '牌局已结束，请重新发牌开始新一轮。' : '点击“重新发牌”开始游戏');}}
            players={players}
