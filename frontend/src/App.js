...
function App() {
  ...
  // --- (Helper functions 和 useEffects - 保持不变，此处省略，确保它们在你的文件中是完整的) ---
  ...
  useEffect(() => { if (showComparisonModal) { setGameMessage(''); return; } if (!gameStarted && !isLoadingDeal) { setGameMessage(''); return; } if (!humanPlayer) return; const isHumanArranged = checkPlayerFullyArranged(humanPlayer); setHumanMiddleHandLabel(isHumanArranged ? '中道' : '手牌'); let newMessage = ''; if (isHumanArranged) { if (humanPlayer.isMisArranged) { newMessage = "注意：你的牌型组合错误（倒水）！"; } else { const allDone = players.every(p => p.isArranged); if (allDone) { newMessage = "所有玩家已准备好！"; } else { newMessage = "你的牌已摆好！等待AI..."; } } } else if (gameStarted && humanPlayer.initial13Cards && humanPlayer.initial13Cards.length > 0) { newMessage = "请你理牌，AI正在理牌..."; } else if (isLoadingDeal) { newMessage = "正在为所有玩家发牌..."; } setGameMessage(newMessage); }, [humanPlayer, players, checkPlayerFullyArranged, showComparisonModal, isLoadingDeal, gameStarted]);
  ...
  useEffect(() => { if (!gameStarted && !isLoadingDeal && !showComparisonModal ) { setGameMessage(''); } }, [gameStarted, isLoadingDeal, showComparisonModal]);
    ...
    <DragDropContext onDragEnd={onDragEnd}>
      <div className="app-container">
        ...
        <div className={`game-board ${humanPlayer.isMisArranged && humanPlayer.isArranged && !showComparisonModal ? 'misarranged-board' : ''}`}>
          ...
          {/* 2. 牌道区域 */}
          <div className="hand-lanes-container"> {/* 新增一个包裹层，方便与横幅和按钮区隔 */}
          ...
          {/* "开始比牌" 按钮也移到 game-board 内部，位于 controls 下方 */}
        ...
        <ComparisonModal 
            ...
            onClose={() => {setShowComparisonModal(false); /* 不再设置 gameMessage */}}
            players={players}
