...
import ComparisonModal from './components/ComparisonModal'; // 引入模态框
...
const createInitialPlayerState = (id, name, isAI = false) => ({
  ...
  initial13Cards: [],
  isArranged: false, isThinking: false, finalArrangement: null,
  isMisArranged: false, score: 0, roundScore: 0, comparisonResults: {},
});
...
function App() {
  ...
  const [showComparisonModal, setShowComparisonModal] = useState(false); // 控制模态框显示

  // ... (getPlayerDroppableId, checkPlayerFullyArranged, evaluateAndSetPlayerArrangement, useEffects for humanPlayer and allPlayersArranged - 这些基本不变) ...
  // (为了简洁，我将省略之前已提供的且在此次修改中核心逻辑不变的函数，但请确保它们存在于你的App.js中)
  ...
  useEffect(() => { if (!humanPlayer) return; const isHumanArranged = checkPlayerFullyArranged(humanPlayer); setHumanMiddleHandLabel(isHumanArranged ? '中道' : '手牌'); if (isHumanArranged && !showComparisonModal) { if (humanPlayer.isMisArranged) { setGameMessage("注意：你的牌型组合错误（倒水）！"); } else { const allDone = players.every(p => p.isArranged); if (allDone) { setGameMessage("所有玩家已准备就绪！点击“开始比牌”进行结算。"); } else { setGameMessage("你的牌已摆好！等待AI..."); } } } }, [humanPlayer, players, checkPlayerFullyArranged, showComparisonModal]);
  ...
  const handleDealNewHand = async () => {
    ...
    setShowComparisonModal(false); // 关闭比牌模态框
  ...
  const handleStartCompare = () => {
    ...
    setGameMessage("正在计算比牌结果..."); // 这个消息很快会被模态框覆盖
    ...
    const updatedPlayersWithTotalScore = playersAfterComparison.map(p => ({
        ...p,
        score: (p.score || 0) + (p.roundScore || 0), // 确保累加的是本轮得分到总分
        // roundScore 已经由 calculateAllPlayerScores 设置
        // comparisonResults 也已经由 calculateAllPlayerScores 设置
    }));

    setPlayers(updatedPlayersWithTotalScore);
    setShowComparisonModal(true); // 打开比牌结果模态框
  };
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      <div className="app-container">
        <TopBanner 
            aiPlayers={aiPlayers} 
            // handEvaluator 不再需要传递给 TopBanner，因为 AIPlayerDisplay 简化了
        />
        
        <h1 className="game-title">十三水</h1>
        {/* 游戏消息现在主要用于操作反馈，比牌结果在模态框 */}
        ...
        {humanPlayer && typeof humanPlayer.score === 'number' && (
        ...
        <div className="controls">
          ...
          <button className="game-button" onClick={handleHumanPlayerAISort} disabled={isLoadingDeal || humanPlayer.isArranged || humanPlayer.isThinking || showComparisonModal}>
            {humanPlayer.isThinking && humanPlayer.id === HUMAN_PLAYER_ID ? 'AI理牌中...' : 'AI帮我理牌'}
          </button>
          <button className="game-button" onClick={handleConfirmHand} disabled={isLoadingDeal || !humanPlayer.isArranged || (humanPlayer.isMisArranged && humanPlayer.isArranged) || allPlayersArranged || showComparisonModal}>
            确定牌型
          </button>
        </div>
        ...
        <ComparisonModal 
            ...
            players={players} // 传递包含最新分数的 players 数组
