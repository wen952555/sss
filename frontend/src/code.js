...
function App() {
  ...
  const handleLogout = async () => { if(currentUser&&currentUser.token){try{await authService.logout(currentUser.token);}catch(e){console.error("Logout mock err:",e);}} setCurrentUser(null); localStorage.removeItem('authToken'); setShowProfilePage(false);};
