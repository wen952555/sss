...
import PlayerHandPyramid from './PlayerHandPyramid';
...
const GameResultModal4P = ({ result, onClose, onPlayAgain, isTrial = false, user }) => {
    ...
    const me = sortedPlayers.find(p => p.id === user.id) || sortedPlayers[0];
    ...
    return (
        <div className="result-modal-backdrop">
            <div className="result-modal-container">
                ...
                <div className="result-players-scroll-container" ref={scrollContainerRef} onScroll={handleScroll}>
                    {sortedPlayers.map((player) => {
                        ...
                        return (
                            <div key={player.id} className="player-result-page">
                                <div className={`player-result-row ${isMe ? 'is-me' : ''}`}>
                ...
                <div className="pagination-dots">
                    {sortedPlayers.map((_, index) => (
                        <span key={index} className={`dot ${currentPage === index ? 'active' : ''}`}></span>
                    ))}
                </div>
