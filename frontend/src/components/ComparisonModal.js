...
// HAND_TYPE_NAMES 如果 scoreCalculator.js 生成的 details 中不包含牌型名称，则不需要
...
const PlayerComparisonCell = ({ player, isHuman, players }) => {
    ...
    // 为了让卡牌尺寸能自适应田字格单元格，我们将在这里动态计算或依赖CSS
    const renderPlayerHandCards = (cards,道) => {
        // 卡牌尺寸现在主要由CSS控制，这里只负责渲染
        return (
            <div className="mini-cards"> {/* CSS会控制这里的布局和卡牌尺寸 */}
                {cards.map(c => (
                    <StaticCard 
                        key={`${c.id}_modal_static_${道}`}
                        cardData={c}
                        // style prop 不再由这里传递固定尺寸，尺寸由CSS的 .comparison-cell .static-card 控制
                    />
                ))}
            </div>
        );
    ...
    return (
        <div className={`comparison-cell ${isHuman ? 'human-player-cell' : 'ai-player-cell'}`}>
            ...
            <div className="comparison-hand-verticals">
                <div className="comparison-hand-row"> <strong>头道:</strong> {renderPlayerHandCards(topCards, 'top')} </div>
                <div className="comparison-hand-row"> <strong>中道:</strong> {renderPlayerHandCards(middleCards, 'middle')} </div>
                <div className="comparison-hand-row"> <strong>尾道:</strong> {renderPlayerHandCards(bottomCards, 'bottom')} </div>
            </div>
            {player.comparisonResults && Object.entries(player.comparisonResults).map(([opponentId, res]) => { 
                ...
                if (res.details && Array.isArray(res.details) && res.details.some(d => d.toLowerCase().includes("打枪")) && res.score !== 0) { 
                    return ( <p key={`${player.id}_vs_${opponentId}_shoot`} className={`shoot-info ${res.score > 0 ? 'positive-shoot' : 'negative-shoot'}`}> {res.score > 0 ? `打枪 ${opponent?.name || '对手'}!` : `被 ${opponent?.name || '对手'} 打枪!`} </p> ); 
                } 
                return null; 
            })}
        </div>
...
const get2x2GridOrder = (players, humanPlayerId) => {
  ...
  return [ order[0], order[1], order[2], order[3] ]; // 直接返回一个扁平数组
};

const ComparisonModal = ({ onClose, players, humanPlayerId, onContinueGame }) => {
  if (!players || players.length === 0) return null;
  const orderedPlayers = get2x2GridOrder(players, humanPlayerId);
  ...
  return (
    <div className="comparison-view-container"> 
      <div className="modal-content comparison-modal-content">
        ...
        <h2>本局比牌结果</h2>
        <div className="comparison-grid"> {/* 这个div将应用CSS Grid */}
