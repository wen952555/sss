<template>
  <div class="game-setup-panel">
    ...
    <div v-if="gameStore.gameId && gameStore.gameState?.status === 'waiting'">
      ...
      <p>玩家列表:</p>
      <ul>
        <li v-for="player in gameStore.players" :key="player.id">{{ player.name }} {{ player.is_me ? '(你)' : '' }}</li>
      </ul>
      <button
    ...
    <div v-if="gameStore.loading && !gameStore.error" class="loading-message">操作中，请稍候...</div>
  </div>
</template>
...
// 尝试从 localStorage 获取昵称，或生成一个随机的
...
onMounted(() => {
    // 如果 store 中有 gameId 但没有 gameState (例如页面刷新)，尝试获取状态
