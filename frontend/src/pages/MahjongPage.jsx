...
const MahjongPage = () => {
  ...
  useEffect(() => {
    ...
    const drawTileForCurrentPlayer = () => {
      ...
      if (currentPlayer === 1) {
        setDrawnTile(tile);
        if (mahjongLogic.isWinningHand([...hands[1], tile])) {
          setWinner(1);
        }
      } else {
        const aiHand = hands[currentPlayer];
        const handWithDrawnTile = [...aiHand, tile];
    ...
    if (currentPlayer !== 1) {
      const timer = setTimeout(drawTileForCurrentPlayer, 1000);
      return () => clearTimeout(timer);
    } else if (!drawnTile) {
      drawTileForCurrentPlayer();
    }
  }, [currentPlayer, winner, wall]);
  ...
  return (
    <div className="mahjong-page">
      ...
      <div className="game-header">
        <h1>麻将</h1>
        <button onClick={startNewGame} className="new-game-button">新游戏</button>
      </div>
      <div className="game-board">
        ...
        <div className={`player-area player-1 ${currentPlayer === 1 ? 'active' : ''}`}>
          ...
          <div className="hand-container">
            {hands[1].map(tile => <Tile key={tile.id} tile={tile} onClick={handleDiscardTile} />)}
            {drawnTile && currentPlayer === 1 && <div className="drawn-tile-separator" />}
            {drawnTile && currentPlayer === 1 && <Tile key={drawnTile.id} tile={drawnTile} onClick={handleDiscardTile} />}
          </div>
