<?php
// backend/utils/GameUtils.php

function dealCards($gameType, $playerCount, $gameMode) {
    $ranks = ['2', '3', '4', '5', '6', '7', '8', '9', '10', 'jack', 'queen', 'king', 'ace'];
    $suits = ['spades', 'hearts', 'clubs', 'diamonds'];
    $deck = [];

    if ($gameMode === 'multiplayer' && $gameType === 'thirteen') {
        // Standard deck
        foreach ($suits as $suit) foreach ($ranks as $rank) $deck[] = ['rank'=>$rank, 'suit'=>$suit];
        // Add extra cards for additional players
        $extraPlayers = $playerCount - 4;
        for ($i = 0; $i < $extraPlayers; $i++) {
            foreach ($ranks as $rank) {
                // Add one of each rank, with a placeholder suit
                $deck[] = ['rank' => $rank, 'suit' => 'extra' . ($i + 1)];
            }
        }
    } else {
        // Standard deck
...
const SSS_SCORES = [
    'HEAD' => ['三条' => 3],
    'MIDDLE' => ['五条' => 12, '铁支' => 8, '同花顺' => 10, '葫芦' => 2],
    'TAIL' => ['五条' => 6, '铁支' => 4, '同花顺' => 5],
