...
require_once '../utils/CardComparator.php'; // Include the comparator
...
switch ($action) {
    case 'get_game':
        ...
        if ($game) {
            echo json_encode(['game_id' => $game['id'], 'player_hand' => json_decode($game['player1_original']), 'opponents' => [['id' => 2, 'name' => 'Opponent 2'], ['id' => 3, 'name' => 'Opponent 3'], ['id' => 4, 'name' => 'Opponent 4']]]);
        } else {
    ...
    case 'submit':
        ...
        if ($submissionId) {
            // After submission, try to calculate scores
            $results = $submissionModel->calculateScores($data['game_id'], $comparator);
            echo json_encode(['message' => 'Submission successful', 'results' => $results]);
        } else {
    ...
    case 'get_results':
        ...
        $results = $submissionModel->findByGame($gameId);
